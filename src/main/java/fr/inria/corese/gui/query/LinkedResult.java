...
/**
 * Query Results contain link URLs with intermediate query and result Load and parse document at
 * URLs Display query in new query panel and display results below use case: federated service with
 * URL parameter mode=why
 */
public class LinkedResult implements URLParam {
    ...
    /**
     * When Mappings contains explain link Parse intermediate query/result Display query/result in
    ...
    /** json is a JSON representation of server Context it contains URL parameters such as mode */
    ...
    void analyseSelection() {
        ...
        for (Expression exp :
                getSourceSelectionAST().getUndefinedTriple(getSourceSelectionMappings())) {
    ...
    /** url: query linked document uri: endpoint */
    ...
    /**
     * copy last query, rewrite body with service clause display it in new panel such that it can be
     * executed as is
     */
    ...
    /** ast = select where {} -> select where { service url { ast }} */
