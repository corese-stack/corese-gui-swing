 ...
 * @author edemairy
 */
public class SparqlQueryEditor extends JPanel
        implements Runnable, ActionListener, DocumentListener, FocusListener, CaretListener {
    ...
    private static char getCharAt(Document doc, int p) throws BadLocationException {
        return doc.getText(p, 1).charAt(0);
    }

    private static int findMatchingParen(Document d, int paren) throws BadLocationException {
        int parenCount = 1;
        int i = paren - 1;
        for (; i >= 0; i--) {
    ...
    private void search() {
            ...
            for (int i = temp; i < d.getLength(); i++) {
                ...
                if (b_word || b_wordUpper) {
                    if (b_word == true) {
                        start =
                                content.toString()
                                        .indexOf(kw); // stocke dans le tableau afin de le surligner
                        // plus tard
                        assert (start > -1);
        ...
        for (int i = temp3;
            ...
            if (b_quote == true) {
                ...
                content =
                        (content.subSequence(
                                startQuote + 1,
                                content.length())); // decoupe la chaine pour trouver le prochain
                // guillemet
            }
        }
        try {
                ...
                if (b_function || b_functionUpper) {
                    if (b_function) {
                        start =
                                content.toString()
                                        .indexOf(
                                                fun); // stocke dans le tableau afin de le surligner
                        // plus tard
                        assert (start > -1);
                        ...
                        start =
                                content.toString()
                                        .indexOf(fun.toUpperCase()); // stocke dans le tableau
                        // afin de le surligner
                        // plus tard
                        assert (start > -1);
        ...
        for (int i = 0; i < tabStartWord.size(); i++) {
            textPaneQuery
                    ...
                    .setCharacterAttributes(
                            tabStartWord.get(i),
                            tabStopWord.get(i) - tabStartWord.get(i),
                            style,
                            false);
        }
        for (int i = 0; i < tabStartFunction.size(); i++) {
            textPaneQuery
                    .getStyledDocument()
                    .setCharacterAttributes(
                            tabStartFunction.get(i),
                            tabStopFunction.get(i) - tabStartFunction.get(i),
                            function,
                            false);
        }
        for (int i = 0; i < tabStopQuote.size(); i++) {
            textPaneQuery
                    ...
                    .setCharacterAttributes(
                            tabStartQuote.get(i),
                            tabStopQuote.get(i) - tabStartQuote.get(i),
                            quote,
                            false);
        }
        for (int i = 0; i < tabStartComm.size(); i++) {
            textPaneQuery
                    ...
                    .setCharacterAttributes(
                            tabStartComm.get(i),
                            tabStopComm.get(i) - tabStartComm.get(i),
                            comm,
                            false);
        }
    }
    ...
    void search(CharSequence temps, String toSearch, JTextArea textAreaXMLResult, String message) {
        ...
        for (int i = temp; i < temps.length(); i++) {
            ...
            if (b == true) {
                start =
                        temps.toString()
                                .indexOf(toSearch); // stocke dans le tableau afin de le surligner
                // plus tard
        ...
        try {
            for (int i = 0; i < tabStart.size(); i++) {
                textAreaXMLResult
                        .getHighlighter()
                        .addHighlight(
                                tabStart.get(i), tabStop.get(i), DefaultHighlighter.DefaultPainter);
            }
        } catch (BadLocationException e1) {
            e1.printStackTrace();
        }
        if (tabStart.size() == 0 && !toSearch.equals("")) {
            JOptionPane.showMessageDialog(
                    textPaneQuery,
                    toSearch + " n'apparait pas dans le résultat",
                    "info",
